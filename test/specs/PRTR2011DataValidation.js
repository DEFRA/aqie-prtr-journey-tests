...
const reportPrefix = 'PRTR_2011'

const logger = createLogger()

describe('PRTR 2011 Migration Data Validation', () => {
  let facilityLookup

  const excelFile = './test/test-data/PRTR_2011.xlsx'
  const sheetName = '2011_PRTR_DataSet'
  ...
  describe('PRTR 2011 Facilities Data List verification against Production download', () => {
    // -------------------------- Test Case 1 --------------------------------------------//

    it('Should validate PRTR 2011 facilities list against Production Data', async function () {
      this.timeout(0)
      ...
      //
      await SearchByYear.searchByYear('2011')
      ...
      expect(headingText).toContain('Facilities matching 2011')
    ...
    describe('PRTR 2011 Facilities PRTR Data Validation against Production download', () => {
      // -------------------------Test Case2 -----------------------------------------------------//
      facilities.slice(0, facilityLimit).forEach((row) => {
        it(`Should validate PRTR release and transferdata against Facility ${row.FacilityFinal} for 2011`, async function () {
          this.timeout(0)
          ...
          try {
            ...
            await FacilitiesDetailsPage.selectYear('2011')
