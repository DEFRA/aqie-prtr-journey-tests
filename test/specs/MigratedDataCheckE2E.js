...
it('should validate transboundary hazardous waste details', async () => {
   ...
   // await $('#password').setValue('release');
    ...
    await facilityDetailsPage.clickViewDetailsByWasteType('Transboundary hazardous waste');
    ...
    await transboundaryPage.validateReceiverCompany([
        'Portoesme s.r.l',
        'Italy'
    ]);
...
it('should verify download options for all years', async () => {
   ...
   // ===== Step 1: Search for location =====
    ...
    await expect(
        await downloadPage.isYearAvailable('2024')
    ).toBe(true);
