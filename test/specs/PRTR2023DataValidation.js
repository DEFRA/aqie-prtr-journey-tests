...
const reportPrefix = 'PRTR_2023'

const logger = createLogger()

describe('PRTR 2023 Migration Data Validation', () => {
  let facilityLookup

  const excelFile = './test/test-data/PRTR_2023.xlsx'
  const sheetName = '2023_PRTR_DataSet'
  ...
  describe('PRTR 2023 Facilities Data List verification against Production download', () => {
    // -------------------------- Test Case 1 --------------------------------------------//

    it('Should validate PRTR 2023 facilities list against Production Data', async function () {
      this.timeout(0)
      ...
      //
      await SearchByYear.searchByYear('2023')
      ...
      expect(headingText).toContain('Facilities matching 2023')
    ...
    describe('PRTR 2023 Facilities PRTR Data Validation against Production download', () => {
      // -------------------------Test Case2 -----------------------------------------------------//
      facilities.slice(0, facilityLimit).forEach((row) => {
        it(`Should validate PRTR release and transferdata against Facility ${row.FacilityFinal} for 2023`, async function () {
          this.timeout(0)
          ...
          try {
            ...
            await FacilitiesDetailsPage.selectYear('2023')
