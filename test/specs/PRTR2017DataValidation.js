...
const reportPrefix = 'PRTR_2017'

const logger = createLogger()

describe('PRTR 2017 Migration Data Validation', () => {
  let facilityLookup

  const excelFile = './test/test-data/PRTR_2017.xlsx'
  const sheetName = '2017_PRTR_DataSet'
  ...
  describe('PRTR 2017 Facilities Data List verification against Production download', () => {
    // -------------------------- Test Case 1 --------------------------------------------//

    it('Should validate PRTR 2017 facilities list against Production Data', async function () {
      this.timeout(0)
      ...
      //
      await SearchByYear.searchByYear('2017')
      ...
      expect(headingText).toContain('Facilities matching 2017')
    ...
    describe('PRTR 2017 Facilities PRTR Data Validation against Production download', () => {
      // -------------------------Test Case2 -----------------------------------------------------//
      facilities.slice(0, facilityLimit).forEach((row) => {
        it(`Should validate PRTR release and transferdata against Facility ${row.FacilityFinal} for 2017`, async function () {
          this.timeout(0)
          ...
          try {
            ...
            await FacilitiesDetailsPage.selectYear('2017')
